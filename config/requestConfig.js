...
let $http = new request({
	...
	header: {
		...
		'token': uni.getStorageSync("token"), //项目token（可删除）
	}
});
// 添加获取七牛云token的方法
...
$http.requestStart = function(options) {
	...
	if (options.load) {
		//打开加载动画
		store.commit("common/setLoadingShow", true);
	}
	// 图片上传大小限制
...
$http.requestEnd = function(options, resolve) {
	...
	if (options.load) {
		// 关闭加载动画
		store.commit("common/setLoadingShow", false);
	}
}
let loginPopupNum = 0;
...
$http.dataFactory = async function(res) {
	...
	if (res.response.statusCode && res.response.statusCode == 200) {
		...
		if (httpData.success || httpData.code == 200) {
		...
		} else if (httpData.code == "1000" || httpData.code == "1001" || httpData.code == 1100 || httpData.code == 401) {
			// 失败重新请求（最多重新请求3次）
			// if(res.resend < 3){
			...
			store.commit("user/emptyUserInfo");
			// #ifdef MP-WEIXIN
			// onLogin();
			// #endif
			// #ifdef H5
